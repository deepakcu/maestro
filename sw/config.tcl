...
# =============================
# if you ever change the SOPC address just change the MAC base address here. 
# This will reflect in MAC/PCS/PHY config files.
#http://stackoverflow.com/questions/10165513/defining-and-looping-through-arrays-tcl
set mac_base_list {};
lappend mac_base_list 0x20000000;
lappend mac_base_list 0x20000400;
lappend mac_base_list 0x20000800;
#lappend mac_base_list 0x20000c00;		
...
set mac_addr_low_list {};
set MAC_0 				0x32464e00; #00:4e:46:32:43:00
lappend mac_addr_low_list 0x00000043;
lappend mac_addr_low_list 0x00000143;
lappend mac_addr_low_list 0x00000243;
lappend mac_addr_low_list 0x00000343;
...
set phy_addr_list {};
lappend phy_addr_list 0;
lappend phy_addr_list 1;
lappend phy_addr_list 2;
lappend phy_addr_list 3;
...
#for {set i 0} {$i < [length $mac_base_list]} {incr i} {
for {set i 0} {$i < 3} {incr i} {
    #puts "${i}=[lindex $mylist $i]"
    set MAC_BASE_ADDRESS [lindex $mac_base_list $i];
    set PHY_ADDR [lindex $phy_addr_list $i];
    set MAC_1 [lindex $mac_addr_low_list $i];    

#    set MAC_BASE_ADDRESS [lindex $mac_base_list 0];
#    set PHY_ADDR [lindex $phy_addr_list 0];
#    set MAC_1 [lindex $mac_addr_low_list 0];    
...
set REV_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0x0}];
set MAC_SCRATCH_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x4}];
set COMMAND_CONFIG_ADDRESS		[expr {$MAC_BASE_ADDRESS + 0x8}];
set MAC_0_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0xC}];
set MAC_1_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0x10}];
set FRM_LENGTH_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x14}];
set PAUSE_QUANT_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x18}];
set RX_SECTION_EMPTY_ADDRESS	[expr {$MAC_BASE_ADDRESS + 0x1C}];
set RX_SECTION_FULL_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x20}];
set TX_SECTION_EMPTY_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x24}];
set TX_SECTION_FULL_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x28}];
set RX_ALMOST_EMPTY_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x2C}];
set RX_ALMOST_FULL_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x30}];
set TX_ALMOST_EMPTY_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x34}];
set TX_ALMOST_FULL_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x38}];
set MDIO_ADDR0_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x3C}];
set MDIO_ADDR1_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x40}];
set REG_STATUS_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x58}];
set TX_IPG_LENGTH_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x5C}];
set TX_CMD_STAT_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0xE8}];
set RX_CMD_STAT_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0xEC}];
set MAC_ADDRESS_NEXT 			[expr {$MAC_BASE_ADDRESS + 0x400}];
...
set PHY_CONFIG_ADDRESS 					[expr {$MAC_BASE_ADDRESS + 0x40}];  
set PHY_CONTROL_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0x280}];
set PHY_ADVERTISEMENT_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x290}];
set PHY_1000_BASE_T_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x2A4}];
set PHY_SPECIFIC_CONTROL_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x2C0}];
set PHY_EXD_SPECIFIC_STATUS_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x2EC}];
set PHY_SPECIFIC_STATUS_ADDRESS 		[expr {$MAC_BASE_ADDRESS + 0x2C4}];
...
set PCS_REV_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0x244}];
set PCS_SCRATCH_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x240}];
set PCS_IF_MODE_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x250}];
set PCS_CONTROL_ADDRESS 			[expr {$MAC_BASE_ADDRESS + 0x200}];
set PCS_STATUS_ADDRESS 				[expr {$MAC_BASE_ADDRESS + 0x204}];
set PCS_PARTNER_ABILITY_ADDRESS 	[expr {$MAC_BASE_ADDRESS + 0x214}];
